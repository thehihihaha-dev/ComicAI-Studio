...
import { sanitizeQueue } from "./queuePrivacy";
...
test("queue sanitizer strips every prediction-derived field", () => {
    ...
    items: [
      {
        ...
        image_dimensions: { width: 900, height: 1280 },
        human_panels: [],
        image_url: "/image",
        candidates: [1],
        selected_panels: [1],
        detected_panel_count: 4,
        ambiguous_panel_count: 2,
        unresolved: true,
        confidence: 0.9,
        iou: 1,
        rejection_reasons: ["x"],
  ...
  assert.equal(sanitized.total, 6);
});
